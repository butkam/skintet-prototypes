...
const ctaLabel = computed(() =>
  installments.value
    ? `Оформити · сьогодні ${formatPrice(schedule.value.today)}`
    : `Оплатити · ${formatPrice(cart.total.value)}`,
)
              ...
              <div v-if="installments" class="installments__body">
                ...
                <article v-for="g in groups" :key="g.kind" class="plan">
                  ...
                  <div class="plan__controls">
                    <SkSegmented v-model="plans[g.kind].payments" :options="PAYMENT_OPTIONS[g.kind]" label="Кількість платежів" />
                ...
                <section class="schedule" aria-labelledby="schedule-title">
                  ...
                  <p v-if="isSplit" class="schedule__note body-s">
...
/* Figma 112:5388: full-bleed neutral/50 band behind the plans, fading at both ends */
.installments.is-open::before {
  content: '';
  position: absolute;
  top: 36px;
  bottom: -32px;
